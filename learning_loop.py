...
Runs repeatedly, learning from trades, updating shared state
"""
...
class Config:
    def __init__(self):
        ...
        self.max_runtime_seconds = int(os.getenv('MAX_RUNTIME_SECONDS', '21000'))
        self.safety_buffer_seconds = 300
        ...
        for dir_path in [self.data_dir, self.results_dir, self.logs_dir, self.state_dir]:
    ...
    def load_state(self):
        ...
        try:
            ...
            if params_file.exists():
                ...
                # Load trades
                trades_file = self.state_dir / 'recent_trades.json'
    ...
    def time_remaining(self) -> float:
        ...
        return max(0, self.max_runtime_seconds - elapsed - self.safety_buffer_seconds)
...
def setup_logging(config: Config):
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(config.logs_dir / 'learning_loop.log'),
...
class LearningEngine:
    ...
    def learn_from_trades(self, recent_trades: List[Dict]):
        """Learn from recent trades and update parameters"""
        if len(recent_trades) < self.config.min_trades_before_learning:
            return
        ...
        logger.info(f"\n🧠 Learning update:")
        logger.info(f"   Win rate: {win_rate:.1f}% | PF: {profit_factor:.2f}")
        for param, value in changes.items():
...
class BacktestEngine:
    ...
    def run_iteration(self, data_pool: Dict) -> Dict:
        all_trades = []
        ...
        if all_trades:
            self.learning_engine.learn_from_trades(all_trades)
            self.config.total_trades_analyzed += len(all_trades)
...
def main():
    try:
        logger.info("=" * 80)
        logger.info("CONTINUOUS LEARNING LOOP")
        ...
        while config.should_continue():
            ...
            logger.info(f"\n🔄 Iteration {config.iteration} | Time remaining: {config.time_remaining()/3600:.1f}h")
            
            metrics = backtest.run_iteration(data_pool)
        ...
        logger.info(f"Final params: {config.learned_params}")
        logger.info("=" * 80)

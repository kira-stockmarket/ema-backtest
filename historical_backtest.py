"""
CSV-Driven Backtest
Reads signals from 'Backtest Stage 2 weekly.csv', verifies Daily EMA Contraction, 
and executes the portfolio simulation with 10% Risk Sizing.
"""

import pandas as pd
import numpy as np
import yfinance as yf
from datetime import datetime
import os
import warnings

warnings.filterwarnings('ignore')

# --- CONFIGURED PARAMETERS ---
PARAMS = {
    'broom_compression_threshold': 0.15,
    'base_duration_min': 20,       # Relaxed slightly since these are predefined weekly ideas
    'base_duration_max': 250,
    'box_consolidation_height': 0.35, # Relaxed to allow weekly structures
    'stop_loss_buffer': 0.01,
    'measured_move_multiplier': 1.5 
}

INITIAL_CAPITAL = 100000.0
POSITION_SIZE_PCT = 0.10  # Deploy 10% of equity per trade
MAX_POSITIONS = 10
CSV_FILENAME = "Backtest Stage 2 weekly.csv"

def backtest():
    if not os.path.exists(CSV_FILENAME):
        print(f"Error: {CSV_FILENAME} not found!")
        return

    print(f"Loading Weekly Ideas from {CSV_FILENAME}...")
    ideas_df = pd.read_csv(CSV_FILENAME)
    
    # Parse dates (format is DD-MM-YYYY in your CSV)
    ideas_df['Date'] = pd.to_datetime(ideas_df['Date'], format='%d-%m-%Y')
    
    # Extract unique symbols and append '.NS' for Indian stocks
    unique_symbols = ideas_df['Symbol'].unique()
    yf_symbols = [sym + '.NS' for sym in unique_symbols]
    
    print(f"Fetching historical data for {len(yf_symbols)} unique stocks + Nifty 50 Index...")
    tickers_to_fetch = yf_symbols + ['^NSEI']
    data = yf.download(tickers_to_fetch, start="2020-01-01", progress=False)
    
    print("Calculating Macro Regime Filter...")
    nifty_index = data.xs('^NSEI', level=1, axis=1).dropna(how='all').copy()
    nifty_index['EMA_50'] = nifty_index['Close'].ewm(span=50, adjust=False).mean()
    nifty_index['Regime_Bull'] = nifty_index['Close'] > nifty_index['EMA_50']
    
    print("Verifying Daily EMA Contractions for CSV dates...")
    emas = [20, 50, 100, 200]
    signals = []
    
    for index, row in ideas_df.iterrows():
        symbol = row['Symbol']
        yf_symbol = symbol + '.NS'
        target_date = row['Date']
        
        try:
            df = data.xs(yf_symbol, level=1, axis=1).dropna(how='all').copy()
            if df.empty: continue
            
            # Calculate EMAs for this stock
            for p in emas:
                df[f'EMA_{p}'] = df['Close'].ewm(span=p, adjust=False).mean()
            
            # Find the exact date or the closest previous trading day
            past_dates = df.index[df.index <= target_date]
            if len(past_dates) == 0: continue
            calc_date = past_dates[-1]
            
            # Get the row index for structural lookback
            i = df.index.get_loc(calc_date)
            if i < PARAMS['base_duration_max']: continue
            
            current_price = df['Close'].iloc[i]
            ema_200 = df['EMA_200'].iloc[i]
            
            # 1. Price above 200 EMA
            if current_price <= ema_200: continue
            
            # 2. Check Daily EMA Contraction (Compression)
            current_emas = [df[f'EMA_{p}'].iloc[i] for p in emas]
            compression = (max(current_emas) - min(current_emas)) / current_price
            if compression >= PARAMS['broom_compression_threshold']: continue
            
            # 3. Structural Filters (Base Depth)
            lookback = df['High'].iloc[max(0, i - PARAMS['base_duration_max']):i]
            peak_price = lookback.max()
            peak_idx = df.index.get_loc(lookback.idxmax())
            
            base_duration = i - peak_idx
            if not (PARAMS['base_duration_min'] <= base_duration <= PARAMS['base_duration_max']): continue
            
            base_low = df['Low'].iloc[peak_idx:i+1].min()
            base_depth = peak_price - base_low
            consolidation_height = base_depth / peak_price
            if consolidation_height > PARAMS['box_consolidation_height']: continue
            
            # Setup is valid on this CSV date! Add to signals.
            take_profit = current_price + (PARAMS['measured_move_multiplier'] * base_depth)
            stop_loss = ema_200 * (1 - PARAMS['stop_loss_buffer'])
            
            signals.append({
                'ticker': yf_symbol,
                'date': calc_date,
                'entry_price': current_price,
                'stop_loss': stop_loss,
                'take_profit': take_profit
            })
            
        except Exception as e:
            continue
            
    # Sort signals chronologically
    signals.sort(key=lambda x: x['date'])
    print(f"Filtered {len(ideas_df)} weekly ideas down to {len(signals)} precise Daily EMA Contraction setups.")
    
    # --- SIMULATION ENGINE ---
    print(f"Simulating Portfolio... Starting Cash: ₹{INITIAL_CAPITAL}")
    cash = INITIAL_CAPITAL
    portfolio = {}
    trade_history = []
    equity_curve = []
    
    all_dates = data.index.sort_values().unique()
    signal_idx = 0
    
    for current_date in all_dates:
        current_date_ts = pd.Timestamp(current_date)
        closed_this_day = []
        
        # 1. Update Exits
        for ticker, pos in portfolio.items():
            try:
                day_data = data.xs(ticker, level=1, axis=1).loc[current_date]
                if pd.isna(day_data['Open']): continue
                
                high = day_data['High']
                low = day_data['Low']
                exit_price = None
                
                if high >= pos['take_profit']:
                    exit_price = pos['take_profit']
                elif low <= pos['stop_loss']:
                    exit_price = pos['stop_loss']
                    
                if exit_price:
                    pnl = (exit_price - pos['entry_price']) * pos['shares']
                    cash += (exit_price * pos['shares'])
                    trade_history.append({
                        'Ticker': ticker,
                        'Entry Date': pos['entry_date'].strftime('%Y-%m-%d'),
                        'Exit Date': current_date_ts.strftime('%Y-%m-%d'),
                        'Shares': pos['shares'],
                        'Entry Price': round(pos['entry_price'], 2),
                        'Exit Price': round(exit_price, 2),
                        'Profit/Loss (₹)': round(pnl, 2),
                        'Return (%)': round(((exit_price / pos['entry_price']) - 1) * 100, 2)
                    })
                    closed_this_day.append(ticker)
            except KeyError:
                continue
                
        for ticker in closed_this_day:
            del portfolio[ticker]
            
        # 2. Track daily equity
        daily_equity = cash
        for t, p in portfolio.items():
            try:
                daily_equity += p['shares'] * data.xs(t, level=1, axis=1).loc[current_date]['Close']
            except Exception:
                daily_equity += p['shares'] * p['entry_price']
        
        # 3. Check Macro Regime before entering new setups
        try:
            is_bull_market = nifty_index.loc[current_date, 'Regime_Bull']
        except KeyError:
            is_bull_market = False

        # 4. Enter new setups with 10% Position Sizing
        while signal_idx < len(signals) and signals[signal_idx]['date'] == current_date:
            sig = signals[signal_idx]
            signal_idx += 1
            
            # Only buy if the Nifty 50 is in a bull regime
            if is_bull_market and sig['ticker'] not in portfolio and len(portfolio) < MAX_POSITIONS:
                trade_alloc = daily_equity * POSITION_SIZE_PCT
                
                if cash >= trade_alloc:
                    shares = int(trade_alloc / sig['entry_price'])
                    if shares > 0:
                        cash -= (shares * sig['entry_price'])
                        portfolio[sig['ticker']] = {
                            'entry_date': sig['date'],
                            'entry_price': sig['entry_price'],
                            'stop_loss': sig['stop_loss'],
                            'take_profit': sig['take_profit'],
                            'shares': shares
                        }
                        
        equity_curve.append({'Date': current_date_ts, 'Equity': daily_equity})
        
    # --- CALCULATE METRICS ---
    equity_df = pd.DataFrame(equity_curve).set_index('Date')
    
    final_equity = equity_df['Equity'].iloc[-1]
    years = (all_dates[-1] - all_dates[0]).days / 365.25 if len(all_dates) > 0 else 1
    cagr = ((final_equity / INITIAL_CAPITAL) ** (1 / years)) - 1
    
    equity_df['Peak'] = equity_df['Equity'].expanding(min_periods=1).max()
    equity_df['Drawdown'] = (equity_df['Equity'] / equity_df['Peak']) - 1
    max_drawdown = equity_df['Drawdown'].min()
    calmar_ratio = cagr / abs(max_drawdown) if max_drawdown != 0 else float('inf')
    
    print(f"\n--- HYBRID CSV BACKTEST RESULTS ---")
    print(f"Total Trades Executed: {len(trade_history)}")
    print(f"Final Portfolio Value: ₹{final_equity:,.2f}")
    print(f"CAGR: {cagr * 100:.2f}%")
    print(f"Max Drawdown: {max_drawdown * 100:.2f}%")
    print(f"Calmar Ratio: {calmar_ratio:.2f}")
    
    if len(trade_history) > 0:
        pd.DataFrame(trade_history).to_csv('csv_backtest_trades.csv', index=False)
        print("Saved: 'csv_backtest_trades.csv'")
    
if __name__ == "__main__":
    backtest()
